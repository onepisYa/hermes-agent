...
class FeishuAdapter(BasePlatformAdapter):
    ...
    def _handle_approval_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        if CallBackCard is not None:
            card = CallBackCard()
            card.type = "raw"
            card.data = json.dumps(self._build_resolved_approval_card(choice=choice, user_name=user_name), ensure_ascii=False)
            response.card = card
        return response
    ...
    def _handle_update_prompt_card_action(self, *, event: Any, action_value: Dict[str, Any], loop: Any) -> Any:
        ...
        if CallBackCard is not None:
            card = CallBackCard()
            card.type = "raw"
            card.data = json.dumps(self._build_resolved_update_prompt_card(answer=answer, user_name=user_name), ensure_ascii=False)
            response.card = card
        return response
